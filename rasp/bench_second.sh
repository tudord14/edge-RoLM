...
for m in "${MODELS[@]}"; do
  curl -sS --connect-timeout 3 --max-time 5 "$HOST/api/generate" \
    ...
    -d '{"model":"'"$m"'","prompt":"warmup","stream":false,"options":{"num_ctx":512,"num_predict":4,"num_thread":2},"keep_alive":0}' >/dev/null || true
done
for m in "${MODELS[@]}"; do
  curl -sS --connect-timeout 5 --max-time 8 "$HOST/api/generate" \
    -H "Content-Type: application/json" \
    -d '{"model":"'"$m"'","prompt":"warmup 4k","stream":false,"options":{"num_ctx":4096,"num_predict":8,"num_thread":2},"keep_alive":0}' >/dev/null || true
done
...
for model in "${MODELS[@]}"; do
  for th in "${THREADS[@]}"; do
    for ctx in "${CTX[@]}"; do
      ...
      P_JSON=$(jq -Rs . <<<"$base_prompt")
      SYS_JSON=$(jq -Rs . <<<"$FIXED_SYS")
      for pred in "${PRED[@]}"; do
