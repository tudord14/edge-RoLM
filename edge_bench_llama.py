...
def build_prompt_of_tokens(llm: Llama, target_tokens: int) -> str:
    ...
    toks = []
    seed_toks = llm.tokenize(seed.encode("utf-8"))
...
def long_context_stress(model_path: str, n_ctx: int, seed: int = 123) -> str:
    # Set CUDA_VISIBLE_DEVICES environment variable to specify GPU usage
    if os.getenv('CUDA_VISIBLE_DEVICES') is None:
        os.environ['CUDA_VISIBLE_DEVICES'] = "0"  # Set to "0" for the first GPU, or modify based on your setup

    try:
        # Initialize Llama with correct arguments
        llm = Llama(
            model_path=model_path,
            n_ctx=n_ctx,
            logits_all=False,
        ...
        return f"Error during Llama initialization: {str(e)}"
...
def main():
    ...
    with open(args.out_csv, "a", newline="", encoding="utf-8") as f:
        ...
        for mp in args.models:
            ...
            stress = long_context_stress(mp, args.n_ctx)  # Remove gpu_layers and use_gpu
